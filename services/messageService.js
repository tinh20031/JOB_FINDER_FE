...
const messageService = {
  ...
  getMessagedCandidates: (companyId) => {
    return axios.get(`${API_URL}/candidates-messaged/${companyId}`, { headers: getHeaders() });
  },

  // Get list of companies that a candidate has messaged
  getMessagedCompanies: (candidateId) => {
    return axios.get(`${API_URL}/companies-messaged/${candidateId}`, { headers: getHeaders() });
  },

  // Get message history between two users
  getMessageHistory: (userId, partnerId) => {
    return axios.get(`${API_URL}/history/${userId}/${partnerId}`, { headers: getHeaders() });
  },

...
const CoreSkillsModal = ({ open, onClose, initialSkills }) => {
  ...
  const handleSave = async () => {
    // If we have a pending skill, show confirmation dialog
    if (pendingSkill) {
    ...
    await saveSkills(false);
  };

  const saveSkills = async (includePending = false) => {
    setSaving(true);
    try {
      // Create a copy of the skills array to work with
      let skillsToSave = [...skills];

      // If we should include the pending skill, add it to the array
      if (includePending && pendingSkill) {
        // Check max limit
        if (skills.length >= 20) {
          toast.warn("You can add a maximum of 20 skills.");
          setSaving(false);
        ...
        // Add pending skill to skills to save
        skillsToSave.push({
          ...pendingSkill,
          skillId: `new_${Date.now()}`,
          isPending: false,

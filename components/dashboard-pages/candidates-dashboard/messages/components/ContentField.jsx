...
const ChatBoxContentField = ({ 
  ...
  const handleCompanyNameClick = () => {
    if (currentChatPartner && currentChatPartner.id) {
      router.push(`/employers-single-v1/${currentChatPartner.id}`);
    }
  };
      ...
      <div className="card-body msg_card_body" ref={messagesContainerRef} style={{ overflowY: 'auto', maxHeight: 'calc(100vh - 300px)' }}>
        ...
        {loadingMessages ? (
          ...
          messages.map((msg, index) => {
            ...
            // Nếu là tin nhắn của mình (ứng viên gửi)
            let avatarUrl, displayName;
            if (isMyMessage) {
              avatarUrl = (currentUserProfileImage && (currentUserProfileImage.startsWith('/') || currentUserProfileImage.startsWith('http'))) ? currentUserProfileImage : '/images/resource/candidate-6.png';
              displayName = currentUserFullName || 'You';
            } else {
              // Nếu là tin nhắn nhận từ công ty, dùng avatar và name của công ty (giống sidebar)
              avatarUrl = (currentChatPartner && currentChatPartner.avatar && (currentChatPartner.avatar.startsWith('/') || currentChatPartner.avatar.startsWith('http')))
                ? currentChatPartner.avatar
                : '/images/resource/default-avatar.png';
              displayName = (currentChatPartner && currentChatPartner.name) || '';
            }
